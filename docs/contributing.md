...
Este repositório deve funcionar como a fonte principal de documentação do projeto Toque de Mulher. Qualquer contribuição deve manter coerência entre **planejamento**, **documentação** e **código versionado**.

## Regras gerais

- escreva em português claro e objetivo;
- prefira atualizar a documentação existente em vez de criar arquivos paralelos;
- não recrie conteúdo redundante em pastas como `Documentacao/`;
- sempre explicite quando o plano e o código atual ainda não estão alinhados.
...
Antes de abrir uma revisão:

- confira se os arquivos alterados continuam coerentes entre si;
- confirme que datas e status refletem o snapshot correto;
- descreva no PR se a mudança é estratégica, técnica ou apenas editorial.
